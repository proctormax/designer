qPack("ui.layouts.CLayout",function(){

const CList = qRequire("CList");
const CStyle = qRequire("CStyle");
const CLayoutItem = qRequire("CLayoutItem");
const CSpacerItem = qRequire("CSpacerItem");
	/***
	 * CLayout
	 * The base class of geometry managers
	**/
	//QB.RequireClass("qb.gui.CWidget");
	const TAG_NAME  = 'CLayout';
	const TAG_GROUP = 'CLayout';
	CCSS.New(TAG_NAME)
	.Add(`.${TAG_GROUP},${TAG_NAME}`,{
		'position':'absolute',
		'display':'block',
		'box-sizing':'content-box',
		'left':'float',
	});
	function CLayout(widget = null, type='' )
	{
		var _dc = DOM(TAG_NAME,`${TAG_GROUP} ${type}`);
		var
		_ = 
		{
			enabled: true,
		...
		this.IsVisible = function(){ return !CCSS.Is(this,"hidden"); };
	...
	CLayout.GetItemRect = function( item ){
		...
		var res 		= {padding:padding, border:border, outlineWidth:outlineWidth, outlineOffset:outlineOffset};
