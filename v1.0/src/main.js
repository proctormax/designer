...
function initAll( ){
  ...
  events    = events(utils.GetTag);
  UI        = UI(events, utils);
  shell     = shell(utils);
  signals   = signals(utils.GetTag,utils);
  ...
  styles    = styles(utils.GetTag);
  transform = transform(utils.GetTag);
  effects   = effects(utils.GetTag);
  colors    = colors(utils.GetTag);
  fills     = fills(utils.GetTag);
  strokes   = strokes(utils.GetTag);
  gradients = gradients(utils.GetTag);
  corners   = corners(utils.GetTag);
  paddings  = paddings(utils.GetTag);
  margins   = margins(utils.GetTag);
  typography= typography(utils.GetTag);
  fonts     = fonts(utils.GetTag);

  board     = utils.GetTag("body");
  header    = utils.GetTag("#header");
  docTabs    = utils.GetTag("#docs");
  content   = utils.GetTag("#content");

  statusbar   = utils.GetTag("#statusBar");
  statusTips  = utils.GetTag("#statusBar #statusTips");
  statusLabel = CText("status","green");
  statusText  = utils.GetTag("#statusText");
  selectionText  = utils.GetTag("#selectionText");
  box       = CTag('box');
  boxCount  = 1;
  /* 
 ...
 initDynamics();
  ...
  initToolsbox();
  initDocTabs();
  parseSlider( utils.GetTag("#shearCombo"));
  initActions(utils.GetTag("#wnd_assets .caction", true));
  initDropZones(utils.GetTag("#wnd_assets .cpane-content", true));
  utils.GetTag('.ui-combo',true).forEach( combo => initComboBox(combo));
  let widget = CText('inline-block');
  let label = CText('display');
  let group = CTag('btn-group');
  
  //utils.Flag(displayCombo,'btn');
  //tabs_menus.states.default.flag = 'fill-dark';

  let textEdit       = new CTextEdit(399,  CTextEdit.Filter_ALPHANUMERIC);
  let intEdit        = new CTextEdit(399,  CTextEdit.Filter_INTEGER);
  let floatEdit      = new CTextEdit(39.9, CTextEdit.Filter_FLOAT);
  //utils.Flag( displayCombo, 'btn');
  addChild( label, group);
  //addChild( displayCombo, $('.cwindow-content'));
  //addChild( group, $('.btn-toolbar'));
...
const newDoc = filename =>{
  let tabText = UI.CText(filename,'ctab','span');
  let tabItem = utils.CTag(null,null,'item');
  addChild(tabText,tabItem);
  addChild(tabItem,docsTabs.children[0]);
  console.warn(filename,' created!!', tabItem);
}
const initDocTabs = ()=>{
  newDoc('home.txt');
}
const initDynamics = ()=>{
  ...
  utils.Flag(utils.GetTag("#menubar"),'stroked-t stroke-accent');
  let tabs_menus    = new UI.CToggleGroup(utils.GetTag("#menubar > div", true),  {
    always:{flag:'padded'},
    //default:  {flag:'stroked-t',   on:'change',  clears:"activated"},
    activated:{flag:'fill-accent text-white', on:'click',  clear:'*'}    
  });
  let tabs_dialog   = new UI.CToggleGroup(utils.GetTag("#dialog > #tabs > .inline", true),  {
    always:  {flag:'stroked-t cursor-default'},
    activated:{flag:'text-white stroke-accent fill-light-', on:'click',  clear:'*'}    
  });
  let tabs  = new UI.CToggleGroup(utils.GetTag(".ui-tabbar > item", true),  {
    always:  {flag:'stroked-l stroke-transparent'},
    activated:{flag:'active', on:'click',  clear:'*', callback: e =>{ }}    
  });
  let toggles  = new UI.CToggleGroup(utils.GetTag(".ui-togglebar > item", true),  {
  ...
  let assets_cats   = new UI.CToggleGroup(utils.GetTag("#dialog .clist > *", true),  {
  ...
  let assets_pages  = new UI.CToggleGroup(utils.GetTag("#wnd_assets .cpane-header", true),  {
  ...
  let assets_actions  = new UI.CToggleGroup(utils.GetTag("#wnd_assets .caction", true),  {
  ...
  let toolsboxStates = new UI.CToggleGroup(utils.GetTag("#toolsbox .caction", true), {
  ...
  let tabs_display  = new UI.CToggleGroup(utils.GetTag("#css_display > div ", true), {
...
const initCommands = ()=>{
   shell.RegisterCommand('select', item => { console.log('selecting item', item);});
   shell.Exec('poly3dSphere -r -t 5 -sx 20 -sy 40 -ax 0 1 1 -cuv -3 -ch 4 -perspective5 0.0 -rotation 0.44');
   let cmd = {
     name: "select",
...
const initLayers = ()=>{
  layers.Add( _stage );
  addChild( layers.pane, utils.GetTag('#layers > content'));
  events.on('layer-selected', e=>{
    console.log( 'layers.Selected',e.layerName );
    _selection.currentItem = e.widget;
    
    utils.ToggleFlag( utils.GetTag(_selection.currentItem), 'selected');
  });
};
...
function initStage(){
  _stage  = utils.GetTag('#stage');
  utils.Flag(_stage,'size-VGA align-hc');
}
///////////// LAYOUTS /////////////////////////////
function initLayouts(){

  let lt = utils.GetTag("layout.ui-togglebar");
  let act_lt_v = utils.GetTag("#act_layout_column");
  let act_lt_h = utils.GetTag("#act_layout_row");
  let act_lt_t = utils.GetTag("#act_layout_toggle");
  ...
  _colors.forEach( (act, index) =>{
    transform.translate(utils.GetTag(act), index%2== 0? 0 : 20, index * 20);
    _makeSelectable(act,true);
    events.on('click', act , e=>{ 
      
      let fill = getComputedStyle( utils.GetTag(act) )['backgroundColor'];
      console.log('color =>',fill);
      utils.GetTag('body').style.backgroundColor = fill; 
    },true);
    AddLayer(act, _stage);
  })
...
function initToolsbox( t=false){
  ...
  let acts = utils.GetTag("#toolsbox .caction", true);
...
function test_drag_n_drop() {
  ...
  utils.Flag(utils.GetTag(dropZone.DC()),'icon-size-small pressed-stroke-dashed hovered-stroke-dotted');
  utils.Flag(utils.GetTag(dropZone2.DC()),'icon-size-medium');
  ...
  events.on('child-pressed', e=>{ 
    utils.SwitchFlags(utils.GetTag(e.target),'stroke-accent','stroke-dark');
...
const dropZone_entered = e =>{
  if( e.buttons && selectedItem !== null ){
    utils.SwitchFlags(utils.GetTag(e.target),'stroke-solid','stroke-dashed');  
...
const dropZone_pressed = e =>{
  if( e.buttons && selectedItem !== null ){
    ...
    utils.ToggleFlag(utils.GetTag(e.target),'fill-base');  
  }
};
...
const drag_droped = e =>{
  ...
  if(item.parentNode !== e.target ){
    ...
    let li = utils.CTag('caction stroked fillet-none', null, utils.GetTag(e.data).tagName);
    //addChild(e.data, li);
    li.innerHTML = utils.GetTag(item).outerHTML;
...
function main(){
  ...
  addChild(mouse.widget,utils.GetTag("#guides"));
