...
function CShell(code = '', name = 'xshell'){
      ...
      <div id="console" class="console" style=" position: relative; height: 100pt; width:110%; overflow-y: auto; bottom: 0 !important; left: 0; ">
          ...
          <div id="input">            
            <div class="code-line prompt">
                ...
                <span id="prompt_sign" style=" color: rgb(1, 93, 171); font-weight: bold;">&gt;</span>
  ...
  const RegType = (type, pattern, callback )=>{
    _types[type] = {type, pattern, callback, exec: new Function(callback) };
    Echo(`defined type ${type}`);
  };
  const ExecType = (type,value)=>{
    let res = null;
    if(type in _types ){
      res = _types[type].exec(value);
      console.log(res);
    }
    Echo(type+" not found");
  }
  
  define(this,'RegType',RegType);
...
/* 
  ...
  const CreatePrompt = ()=>{
        let code = $textEdit.textContent.toString();
        $textEdit.contentEditable = false;
        HistoryPush( code );
        if( !code.endsWith(';'))
        code +=';';
  ...
  RegCommand('type',{name:{type:String, char:'t'}, pattern:{type:RegExp, char:'p'}, callback:{type:String, char:'c'}},RegType);
  RegCommand('exec',{name:{type:String, char:'t'}, value:{type:String, char:'v'}},ExecType);
...
function RegCommands(){
  ...
  shell.RegCommand("text",  {value:{type:String,  char:'v'}, name:{type:String, char:'n'}, parent:{type:String, char:'p'}, tag:{type:String, char:'t'}}, Text);
